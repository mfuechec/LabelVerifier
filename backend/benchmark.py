...
Usage:
    ...
    python benchmark.py --save-golden                # save extractions as golden dataset
    python benchmark.py --diff-golden                # compare extractions against golden dataset
    python benchmark.py --fixture pass-angels-envy --save-golden  # update one fixture
...
GOLDEN_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "tests", "fixtures", "golden_extractions.json",
)
...
def _save_golden(fixtures: list[dict], fixture_extractions: dict[str, list[ExtractionResult]], merger: ImageMerger):
    """Save extraction results as golden dataset to golden_extractions.json."""
    # Load existing golden data to preserve confidences and metadata
    existing = {}
    ...
    golden = {
        "metadata": {
            ...
            "source": "Generated by benchmark.py --save-golden",
            "notes": "LLM-generated draft - review and correct before committing",
        },
        "extractions": existing.get("extractions", {}),
        "extraction_confidences": existing.get("extraction_confidences", {}),
    }
    ...
    print(f"  Updated {updated} fixture(s), total {len(golden['extractions'])} fixture(s)")


def _diff_golden(fixtures: list[dict], fixture_extractions: dict[str, list[ExtractionResult]], merger: ImageMerger):
    """Compare current LLM output against saved golden data."""
    if not os.path.exists(GOLDEN_PATH):
        print("ERROR: No golden dataset found. Run --save-golden first.")
        sys.exit(1)

    with open(GOLDEN_PATH) as f:
        golden = json.load(f)

    golden_data = golden.get("extractions", {})
    diffs_found = 0
    fixtures_compared = 0
    ...
    for fixture_id, extraction_results in fixture_extractions.items():
        ...
        if not golden_fields:
            print(f"\n  {fixture_id}: NOT IN GOLDEN (new fixture)")
            diffs_found += 1
            continue

        fixtures_compared += 1
        field_diffs = []
        all_keys = set(list(current_fields.keys()) + list(golden_fields.keys()))
        for key in sorted(all_keys):
            cur = current_fields.get(key)
            gld = golden_fields.get(key)
            if cur != gld:
                field_diffs.append((key, gld, cur))

        if field_diffs:
            diffs_found += 1
            print(f"\n  {fixture_id}: {len(field_diffs)} diff(s)")
            for fname, gld_val, cur_val in field_diffs:
                gld_short = (str(gld_val)[:60] + "...") if gld_val and len(str(gld_val)) > 60 else str(gld_val)
                cur_short = (str(cur_val)[:60] + "...") if cur_val and len(str(cur_val)) > 60 else str(cur_val)
                print(f"    {fname}:")
                print(f"      golden:  {gld_short}")
                print(f"      current: {cur_short}")

    print(f"\n{'=' * 60}")
    print(f"Compared {fixtures_compared} fixture(s): {diffs_found} with differences")
    print(f"{'=' * 60}")
...
async def main():
    ...
    parser.add_argument("--save-golden", action="store_true", help="Save extractions as golden dataset to tests/fixtures/golden_extractions.json")
    parser.add_argument("--diff-golden", action="store_true", help="Compare current LLM extractions against saved golden dataset")
    ...
    results = []
    fixture_extractions: dict[str, list[ExtractionResult]] = {}  # for golden save/diff
    start = time.time()
    for i, fixture in enumerate(fixtures, 1):
        fid = fixture["id"]
        print(f"[{i}/{len(fixtures)}] {fid}...", end=" ", flush=True)
        try:
            # Extract fields (same as run_single_fixture but we capture raw extractions)
            image_bytes, panels = load_images(fixture)
            extraction_results = []
            for img, panel in zip(image_bytes, panels):
                if cache is not None:
                ...
                if throttle is not None:
                    await throttle.wait()
                t0 = time.time()
                ext_result = await extraction_service.extract_fields(img, panel)
                ...
                if throttle is not None:
                    if elapsed_ext > 10.0:
                        ...
                        throttle.on_success()
                if cache is not None:
                    cache.put(img, EXTRACTION_PROMPT, ext_result)
    ...
    if args.save_golden:
        _save_golden(fixtures, fixture_extractions, merger)
    if args.diff_golden:
        _diff_golden(fixtures, fixture_extractions, merger)
